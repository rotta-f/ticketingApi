...
// Route: /tickets/{id}/close
// Close a ticket
func ArchiveTicket(w http.ResponseWriter, r *http.Request) {
	u := CtxGetValue(r, STORE_AUTH).(*datastructures.User)
	if u == nil {
		utils.WriteError(w, http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError), "")
		return
	...
	if u.Type != datastructures.USER_TYPE_SUPPORT {
		...
		if t.Author.ID != u.ID {
			utils.WriteError(w, http.StatusUnauthorized, http.StatusText(http.StatusUnauthorized), "Client can only archive ticket he owns")
			return
		}
	}
